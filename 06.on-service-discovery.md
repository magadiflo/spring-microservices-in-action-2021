...
En primer lugar, **el service discovery permite al equipo de aplicaciones escalar rápidamente (horizontalmente) la
cantidad de instancias de servicios que se ejecutan en un entorno.** Los consumidores del servicio
...
1. Se comunica con el service discovery para todas las instancias que solicita un consumidor de servicios (cliente) y
   luego almacena en caché los datos localmente en la máquina del consumidor de servicios.
...
- La propiedad `spring.application.instance_id` es una de las propiedades predefinidas de `Spring Boot`. Esta propiedad
  ...
  `servidor Eureka`.
    ...
    - `${random.value}` es una función para generar un valor aleatorio en caso de que las anteriores no estén
      ...
      disponible (por ejemplo, si no estás ejecutando tu aplicación en Cloud Foundry), Spring Boot utilizará
...
instancia `instances.getFirst().getUri().toString()`, es por eso que se comentaba en apartados superiores **(El cliente
Discovery y la vida real)** que usar el `DiscoveryClient` es más para probar, dado que si queremos
