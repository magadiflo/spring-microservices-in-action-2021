...
## Propagando el Access Token

Para demostrar la propagación de un token entre servicios, también protegeremos nuestro servicio de licencia con
Keycloak. Recuerde, el servicio de licencias llama al servicio de la organización para buscar información. La pregunta
es: **¿cómo propagamos el token de un servicio a otro?.**

Vamos a establecer un ejemplo sencillo en el que el servicio de licencias llamará al servicio de la organización. Si
siguió los ejemplos que creamos en los capítulos anteriores, verá que ambos servicios se ejecutan detrás de una puerta
de enlace `(gateway)`. **La `Figura 9.26` muestra cómo el token de un usuario autenticado fluirá a través de la puerta
de enlace, el servicio de licencias y luego hasta el servicio de la organización.**

![65.propagating-a-token.png](./assets/65.propagating-a-token.png)

Esto es lo que ocurre en la `figura 9.26`. Los números correspondientes a los de la `figura 9.26` se indican entre
paréntesis a continuación.

- El usuario ya se ha autenticado con el servidor Keycloak y realiza una llamada a la aplicación web O-stock. El token
  de acceso del usuario se almacena en la sesión del usuario. La aplicación web O-stock necesita recuperar algunos datos
  de licencia y llamar al endpoint REST del servicio de licencias `(1)`. Como parte de la llamada al punto final REST
  de licencia, la aplicación web O-stock agrega el token de acceso a través del encabezado de autorización HTTP. Solo se
  puede acceder al servicio de licencias detrás de Spring Cloud Gateway.


- La puerta de enlace busca el punto final del servicio de licencias y luego reenvía la llamada a uno de los servidores
  del servicio de licencias `(2)`. El servicio de gateway copia el encabezado HTTP de autorización de la
  llamada entrante y garantiza que el encabezado HTTP se reenvíe al nuevo punto final.


- El servicio de licencias recibe la llamada entrante. Debido a que el servicio de licencias es un recurso protegido,
  validará el token con el servidor Keycloak `(3)` y luego verificará las funciones del usuario para conocer los
  permisos adecuados. Como parte de su trabajo, el servicio de licencias invoca el servicio de organización. Al hacer
  esto, el servicio de licencias debe propagar el token de acceso del usuario al servicio de la organización.


- Cuando el servicio de la organización recibe la llamada, toma el encabezado de Autorización HTTP y valida el token
  con el servidor Keycloak `(4)`.

Para implementar estos pasos, necesitamos realizar varios cambios en nuestro código. Si no hacemos esto, obtendremos el
siguiente error al recuperar la información de la organización desde el servicio de licencias:
...
El primer paso es modificar el `gateway` agregando el filtro `RemoveRequestHeader` a cada ruta en el archivo de
configuración `gateway-server.yml` ubicado en el repositorio `Spring Cloud Config`. **Únicamente agregaremos este filtro
para eliminar los encabezados que no queremos que se propagen a los microservicios, ya que como he evidenciado, por
defecto, `Spring Cloud Gateway` propaga los encabezados a los servicios que se llaman.**

Esta configuración es una lista negra de encabezados sensibles que el `gateway` evitará que se propaguen a un servicio
descendente. La ausencia del valor de `Authorization` en la lista `RemoveRequestHeader` significa que el `gateway`
permitirá el paso de ese encabezado.

En resumen el filtro `RemoveRequestHeader` le dice al `gateway` que elimine los encabezados `Cookie` y `Set-Cookie`
de la solicitud durante la operación de enrutamiento, debido a que los servicios posteriores no necesitan esos
encabezados, todo lo que necesitan es el `token de acceso JWT` que va en el encabezado `Authorization`, es por eso que
los encabezados que no coloquemos en la lista sí se van a propagar.
...
> En la `pág. 266` del libro de `Spring Microservices In Action 2021` menciona: **"Si no configuramos esta propiedad de
> configuración [RemoveRequestHeader=Cookie,Set-Cookie], la puerta de enlace bloquea automáticamente la propagación de
> los tres valores (Cookie, Set-Cookie y Autorización)."**, bueno es una afirmación que en la versión actual que estoy
> usando de `Spring Cloud Gateway` no lo he evidenciado, dado que hice las pruebas, incluso en apartados superiores
> podemos ver dichas pruebas; donde el gateway no tenía configurada el filtro `RemoveRequestHeader` e hice una petición
> utilizando gateway al microservicio `organization-service` enviándole en los encabezados el
> `Authorization: Bearer <access_token_jwt>` y el `gateway` lo propagó sin ningún problema, es decir, el microservicio
> `organization-service` recibió el `JWT` en el header `Authorization` que le fue reenviado automáticamente por el
> `Gateway Server`.
>
> `Conclusión`, puedo concluir que el `gateway server` está reenviando sin problemas los encabezados que le llegan
> desde el cliente al microservicio que se esté llamando.
>
> Estoy usando el `RemoveRequestHeader=Cookie,Set-Cookie` para eliminar encabezados que no quiero que se propagen, ya
> que como se mencionó, los servicios a los que se redireccionarán no necesitan dichos encabezados, solo
> necesitan el JWT que va en el enabezdo `Authorization`.

A continuación, debemos configurar nuestro servicio de licencias para incluir la dependencia de
`spring-boot-starter-oauth2-resource-server` y configurar las reglas de autorización que queramos para el servicio.
Finalmente, necesitamos agregar la propiedad `issuer-uri` que apunte a Keycloak en el archivo de propiedades de la
aplicación en el servidor de configuración.

## Protegiendo el servicio de licencia (servidor de recursos)

Al propagar tokens de acceso, nuestro primer paso es agregar la dependencia de Maven a nuestro archivo `pom.xml` del
`licensing-service`. La siguiente lista muestra la dependencia.
...
El siguiente paso es proteger el `servicio de licencias` para que solo pueda acceder a él un usuario autenticado. La
siguiente lista muestra la clase `SecurityConfig` de dicho microservicio:
...
El último paso en la configuración del servicio de licencias es agregar la configuración de Keycloak a nuestro archivo
`licensing-service.yml` ubicada en el servidor de configuraciones. La siguiente lista muestra la configuración que
debemos agregar:
...
## Probando la protección del licensing-service

Realizaremos dos peticiones al servicio de licencia a través del gateway, la primera sin enviar el access token y la
segunda enviándolo, veamos el resultado:

En esta primera petición no le enviamos el access token, y como nuestro microservicio de licencias está protegido, nos
lanza el status `HTTP 401 Unauthorized`:

````bash
$ curl -v http://localhost:8072/license/v1/organization/optimaGrowth/license/a9c7901b-6f0c-4ec7-b42c-fcb483062c17 | jq
>
< HTTP/1.1 401 Unauthorized
< Vary: Origin
< Vary: Access-Control-Request-Method
< Vary: Access-Control-Request-Headers
< Set-Cookie: JSESSIONID=D2337638D6C5E910B052A8056DBFCA7C; Path=/; HttpOnly
...
< X-Frame-Options: DENY
< Content-Length: 0
< Date: Tue, 28 May 2024 23:42:52 GMT
< tmx-correlation-id: e769bff7-a49b-4659-bfab-e52097a751f1
<
````

Ahora, con un `access token` ya generado realizaremos nuevamente la solicitud agregando en el header el `Authorization`
con el `access token`. El `gateway server` reenvía el header al microservicio de licencia quien lo valida utilizando el
servidor de authorización de Keycloak y al ser un `access token` válido, permite que la solicitud continúe y se
retornen los resultados:

````bash
$ curl -v -H "Authorization: Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJRb1RjV09yU3NNR3l3a2hwdW9YR1h3QjVaemUzZUt4SEdzUlVTazVub3NJIn0..IU0sbIVGCI2NBUjFtya-GJjdnhL0aEtk-zLVMfV9HQd6qvGAFYmwOTJli1zdXUEAJdFHM3nbNYPm8z0s2KAWn0qhSvSd-tRISrluUxbkNIUDtlMN2j9f_yFM-xQot0bW5QytT3o5et_xxaTPpWfxniclT2LBX9bv5co8ZegNd_m3gAGoi2RmPOtDFieGNWYn9QvRbKzdmmEsa5qgyT0IHWoi-qY_blUP6UKCh407ibnQ7PP2huXbaDFpGY6dZ9Eg9jTI78qss0R1giuPWSTdVDAuqw11wzHHlG2PKSpgSGnFGVdNJxGokAhaMaaN5Mk7lMe40wu3Ce5PCDLQaoG3XQ" http://localhost:8072/license/v1/organization/optimaGrowth/license/a9c7901b-6f0c-4ec7-b42c-fcb483062c17 | jq
>
< HTTP/1.1 200 OK
...
< Date: Tue, 28 May 2024 23:58:06 GMT
< tmx-correlation-id: a7bd67f1-91eb-4186-8e21-65a4b08d11d2
<
{
  ...
  "_links": {
    "self": {
      "href": "http://192.168.42.204:8080/v1/organization/optimaGrowth/license/a9c7901b-6f0c-4ec7-b42c-fcb483062c17"
    },
    "createLicense": {
      "href": "http://192.168.42.204:8080/v1/organization/{organizationId}/license",
      "templated": true
    },
    "updateLicense": {
      "href": "http://192.168.42.204:8080/v1/organization/{organizationId}/license/optimaGrowth",
      "templated": true
    },
    "deleteLicense": {
      "href": "http://192.168.42.204:8080/v1/organization/{organizationId}/license/optimaGrowth",
...
## [Propagando Bearer Token entre Microservicios](https://docs.spring.io/spring-security/reference/servlet/oauth2/resource-server/bearer-tokens.html)

Ahora que tenemos asegurado nuestro microservicio de licencias, podemos continuar con nuestro paso final, que es
propagar el access token. **¿Por qué necesitamos propagarlo?** Recordemos que nuestro microservicio de licencias se
comunica con el servicio de organización y el servicio de organización está esperando que se le envíe por los headers
el `Authorization` con un `JWT`, si no le envíamos el token, pues nos va a lanzar el error `HTTP 401 Unauthorized`.

Entonces, para propagar el access token todo lo que necesitamos hacer es modificar cómo el código en el servicio de
licencias llama al servicio de la organización. Para eso, debemos asegurarnos de que el encabezado de Authorization
HTTP se inserte en la llamada de la aplicación al servicio de la organización.

Recordemos que estamos usando distintos clientes http para hacer la llamada al microservicio de organización, en este
caos en particular vamos a usar `RestClient` y vamos a modificar el bean creado para agregarle un nuevo interceptor.
Esta forma de propagar lo obtuve de la documentación oficial
...
Crearemos el interceptor en el siguiente directorio del servicio de licencias:
`licensing-service/src/main/java/dev/magadiflo/licensing/app/utils`
...
A continuación modificamos el bean del `RestClient`
`licensing-service/src/main/java/dev/magadiflo/licensing/app/config/AppConfig.java` y le agregamos el interceptor
...
## Probando propagación de token entre microservicios

Para probar el código anterior, podemos realizar un request al servicio de licencias para que llame al servicio de
organización y recupere datos. Por ejemplo, la siguiente solicitud recupera los datos de una licencia específica y
luego obtiene la información asociada para la organización.

````bash
$  curl -v -H "Authorization: Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJRb1RjV09yU3NNR3l3a2hwdW9YR1h3QjVaemUzZUt4SEdzUlVTazVub3NJIn0..9RRaRYgb8lSX_ysguP4sCc6iHBHLwN1CAZ1TUmPf9snAJ9yOTVtamZZYjRNBJJ2DOD-UAT2bF0cCMuRWYqFm0IBD9qIneOPp7ZR_XDh9XDeaWJYcyNH6Pz-q7yKutKP_vGWf8slsGlQR5cq2dT1iPXTj3Zrte5JeqprwrdrEPNLDo88RMamaX2Iqwla0WPqaiWhAZdizJNsEPQrKePnp4nnrppYSsztdKLy9DmWtMXbwo9YGXoYzLUbi_qDTSoi1CXMFJiqV4QBnlKrZS61cbcD_5BZHHbQZhwTpKp51Xli2q70Hb1ALlkNOCjz5fFIpaY3DGjQsQnlapnubp6Hn8g" http://localhost:8072/license/v1/organization/optimaGrowth/license/a9c7901b-6f0c-4ec7-b42c-fcb483062c17/restClient | jq
>
< HTTP/1.1 200 OK
< transfer-encoding: chunked
< Vary: Origin
...
< X-Frame-Options: DENY
< Content-Type: application/hal+json
< Date: Thu, 30 May 2024 00:54:00 GMT
< tmx-correlation-id: 38a4497e-e91f-4cf6-a147-3d4766810453
<
{
  "licenseId": "a9c7901b-6f0c-4ec7-b42c-fcb483062c17",
  "description": "Licencia full",
...
## Analizando un campo personalizado en un JWT

Pasaremos a nuestro `gateway` para ver un ejemplo de cómo analizar un campo personalizado en un `JWT`.
Específicamente, modificaremos la clase `TrackingFilter` que presentamos en el capítulo 8 para decodificar el campo
`preferred_username` del `JWT` que fluye a través del `gateway`. Para hacer esto, utilizaremos una biblioteca
analizadora de JWT y la agregaremos al archivo `pom.xml` del servidor `Gateway`. Hay varios analizadores de tokens
disponibles, pero elegimos el `códec Apache Commons` y el `objectMapper` que viene incluído con Spring Boot.
...
Una vez que hemos agregado la dependencia, podemos crear un nuevo método llamado `getUsername()` al `TrackingFilter`.
La siguiente lista muestra las modificaciones que realizaremos.

Lo primero que realizaremos será modificar la clase `FilterUtils` del `gateway`. Esta clase tendrá un método que nos
retornará la cadena del `Authorization` que es el `Bearer <access_token>`:
...
Como segundo paso, vamos a modificar la clase `TrackingFilter` agregándole métodos para obtener el nombre del usuario
que está almacenado en el atributo `preferred_username` del `payload` del token. Para eso usamos la librería
`códec Apache Commons` y además el `ObjectMapper`, este último lo estamos inyectando vía constructor:
...
Una vez que implementamos la función `getUsername()`, podemos agregar un `log.info()` al método `filter()` en el
`TrackingFilter` para imprimir el `preferred_username` analizado desde nuestro JWT que fluye a través de la
puerta de enlace. Ahora, cuando hagamos una llamada a la puerta de enlace, veremos el valor del `preferred_username` en
la salida de la consola.

````bash
DEBUG d.m.gateway.app.filters.TrackingFilter   : tmx-correlation-id generado en el tracking filter: 4e311bc9-2607-463e-b983-af9be853185a
 INFO d.m.gateway.app.filters.TrackingFilter   : El nombre de la autenticación del token es: illary.huaylupo
````