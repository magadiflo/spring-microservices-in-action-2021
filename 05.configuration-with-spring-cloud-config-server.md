...
Server es cómo pueden actualizar dinámicamente sus aplicaciones cuando cambia una propiedad. No te preocupes. **El
`Config Server` siempre proporciona la versión más reciente de una propiedad.** ¡Los cambios realizados en una
...
utilizando un servidor de configuración que se ejecuta como parte de un proyecto de Spring Boot, **reiniciar el servidor
...
Ahora estamos listos para comenzar a cifrar propiedades para usar con `Spring Cloud Config`. Cifraremos la contraseña
...
`config-repo/licensing-service-dev.yml`
...
Recordemos que cuando consultábamos al servidor de configuraciones algún perfil, debíamos agregarle
...
o.s.c.c.c.ConfigServerConfigDataLoader   : Located environment: name=licensing-service, profiles=[dev], label=null, version=null, state=null
.s.d.r.c.RepositoryConfigurationDelegate : Bootstrapping Spring Data JPA repositories in DEFAULT mode.
.s.d.r.c.RepositoryConfigurationDelegate : Finished Spring Data repository scanning in 102 ms. Found 1 JPA repository interface.
o.s.cloud.context.scope.GenericScope     : BeanFactory id=9f806dfd-573e-3abc-8613-0a58ec5b1e6b
o.s.b.w.embedded.tomcat.TomcatWebServer  : Tomcat initialized with port 8080 (http)
o.apache.catalina.core.StandardService   : Starting service [Tomcat]
o.apache.catalina.core.StandardEngine    : Starting Servlet engine: [Apache Tomcat/10.1.20]
o.a.c.c.C.[Tomcat].[localhost].[/]       : Initializing Spring embedded WebApplicationContext
w.s.c.ServletWebServerApplicationContext : Root WebApplicationContext: initialization completed in 4716 ms
o.hibernate.jpa.internal.util.LogHelper  : HHH000204: Processing PersistenceUnitInfo [name: default]
org.hibernate.Version                    : HHH000412: Hibernate ORM core version 6.4.4.Final
o.h.c.internal.RegionFactoryInitiator    : HHH000026: Second-level cache disabled
o.s.o.j.p.SpringPersistenceUnitInfo      : No LoadTimeWeaver setup: ignoring JPA class transformer
com.zaxxer.hikari.HikariDataSource       : HikariPool-1 - Starting...
com.zaxxer.hikari.pool.HikariPool        : HikariPool-1 - Added connection org.postgresql.jdbc.PgConnection@23dc70c1
com.zaxxer.hikari.HikariDataSource       : HikariPool-1 - Start completed.
org.hibernate.orm.deprecation            : HHH90000025: PostgreSQLDialect does not need to be specified explicitly using 'hibernate.dialect' (remove the property setting and it will be selected by default)
o.h.e.t.j.p.i.JtaPlatformInitiator       : HHH000489: No JTA platform available (set 'hibernate.transaction.jta.platform' to enable JTA platform integration)
Hibernate: 
...
o.s.b.a.e.web.EndpointLinksResolver      : Exposing 19 endpoint(s) beneath base path '/actuator'
o.s.b.w.embedded.tomcat.TomcatWebServer  : Tomcat started on port 8080 (http) with context path ''
d.m.l.app.LicensingServiceApplication    : Started LicensingServiceApplication in 25.076 seconds (process running for 26.166)
````

## Dejando configuraciones solo con perfil native para continuar el curso

Hasta este punto, hemos podido agregar configuraciones a `Vault`, pero, sucede que si el contenedor de vault se detiene
y se vuelve a levantar, las configuraciones definidas se eliminan. Esto ocurre porque no estamos persistiendo la
información almacenada en vault.

**No he investigado mucho sobre cómo hacer persistente las configuraciones almacenadas en vault**, así que para evitar
el inconveniente de estar ingresando las configuraciones cada día que hago el curso, voy a prescindir de usar `Vault`,
así que el `application.yml` del `config-server` ya no tendrá el perfil `vault`, tampoco tendrá el perfil `Git`, ya que
para hacer más sencillo el curso, trabajaré con el perfil `native` definiendo los archivos de configuración en el
directorio `src/main/resources/config` del propio servidor de configuraciones.

Ahora, como aquí estoy documentado el paso a paso del desarrollo del libro, voy a comentar el perfil `vault` y el
perfil `Git` junto a sus configuraciones, mientras que en el archivo real del proyecto sí los voy a eliminar.
...
Además, en el `application.yml` del `license-service` definimos `spring.cloud.config.token`, una propiedad utilizada
para enviar el token de Vault al servidor de configuraciones. Como ahora el `config-server` utiliza el perfil `native`
y ya no obtiene las configuraciones desde `Vault`, esta propiedad deja de ser necesaria. Por ello, la comentaremos
en este archivo para mantenerla como referencia en la documentación, pero la eliminaremos del código fuente.
...
spring:
  ...
  # Token de Vault. Actualmente no requerido al no usar el perfil vault en Spring Cloud Config.
  # cloud:
  #   config:
  #     token: myroot
````
