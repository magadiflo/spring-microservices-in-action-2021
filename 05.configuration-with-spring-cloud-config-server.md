...
## Dejando configuraciones solo con perfil Git para continuar el curso
...
así que el `application.yml` del `config-server` ya no tendrá el perfil `vault`. Ahora, como aquí estoy documentado
el paso a paso del desarrollo del libro, voy a comentar el perfil `vault` y sus configuraciones, mientras que en el
archivo real del proyecto sí lo voy a eliminar.
