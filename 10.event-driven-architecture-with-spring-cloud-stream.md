...
Ahora podemos seguir adelante e implementar el código que publica un mensaje. El primer paso es crear el paquete
`/utils` que contendrá las mismas clases que usamos en el `licensing-sevice`: `UserContext`, `UserContextFilter`,
`UserContextHolder` y un `UserContextInterceptor`. Recordemos que estas clases nos permitirán capturar y manipular
el ID de correlación y otros atributos adicionales.
