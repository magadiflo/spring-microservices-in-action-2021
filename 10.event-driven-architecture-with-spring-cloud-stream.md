...
servicio de licencias. En Spring Initializr podemos encontrar esta dependencia bajo el nombre
...
>
> `Jedis` es un cliente de Redis popular, y si deseamos utilizarlo, necesitamos agregarlo explícitamente como una
> dependencia en el proyecto.

**Nota 2**

> Como usaré la dependencia de `spring-boot-starter-data-redis`, utilizaremos por defecto las dependencias que trae
> consigo: `spring-data-redis` y `lettuce`.
>
> `Lettuce` es un cliente de `Redis` y es la implementación por defecto que viene dentro de la dependencia
> de `spring-boot-starter-data-redis`.

## Construyendo la conexión de la base de datos a un servidor Redis

Ahora que tenemos la dependencia en Maven, necesitamos establecer una conexión con nuestro servidor Redis.
La dependencia agregada en el `pom.xml` utiliza por defecto `Lettuce` para comunicarse con un servidor Redis.
Para comunicarnos con una instancia específica de `Redis`, expondremos una clase `LettuceConnectionFactory` como un
`Bean` de Spring.

Una vez que tengamos una conexión a `Redis`, usaremos esa conexión para crear un objeto `Spring RedisTemplate`.
En mi caso he optado por inyectar el bean `LettuceConnectionFactory` por parámetro para usarlo en el otro bean
del `RedisTemplate`, mientras que en el libro no hacen esa inyección por parámetro, sino que usan directamente
el método `template.setConnectionFactory(lettuceConnectionFactory())`. (Nota. En el libro usan el otro cliente de redis
Jedis`)
...
`El primer archivo` es una interfaz Java que se inyectará en cualquiera de las clases de servicios de licencia que
...
La clase `licensing-service/src/main/java/dev/magadiflo/licensing/app/model/OrganizationModelRedis.java` se muestra
en el siguiente listado. Notar que en mi caso estoy usando el record que hemos venido usando desde el principio.
...
- Una cosa importante a tener en cuenta en el código anterior es que un servidor Redis puede contener múltiples
  hashes y estructuras de datos en su interior. Por lo tanto, debemos decirle a Redis el nombre de la estructura de
  datos sobre la que queremos realizar la operación en cada interacción con Redis.


- La entidad que usemos como modelo para almacenar en la caché de Redis debe tener explícitamente un identificador
  llamado `id` y además estar anotado con `@Id` del paquete `org.springframework.data.annotation.Id`. Si en vez de `id`
  usamos otro nombre, por ejemplo `organizationId` nos va a arrojar un error, ya que `Redis` espera que el campo de
  identificación se llame `id` por defecto cuando se usa la anotación `@RedisHash` en una entidad. La
  anotación `@RedisHash` en Spring Data Redis está diseñada para ser compatible con la convención predeterminada de
  Redis, que busca un campo `id` para identificar las instancias de la entidad en Redis.

## Usando Redis y el servicio de licencia para almacenar y leer datos de la organización

Ahora que tenemos el código implementado para realizar operaciones con Redis, podemos modificar nuestro servicio de
licencias para que cada vez que el servicio de licencias necesite los datos de la organización, verifique el caché de
Redis antes de llamar al servicio de la organización.
...
El método `getOrganization()` es donde se realiza la llamada al servicio de la organización. Antes de realizar la
llamada `REST` real, necesitamos recuperar el objeto `OrganizationModelRedis` asociado con la llamada desde `Redis`
usando el método `checkRedisCache()`.

Si el objeto `OrganizationModelRedis` no está en `Redis`, el código devuelve un valor `null`. Si se devuelve un
valor nulo del método `checkRedisCache()`, el código invoca el punto final `REST` del servicio de la organización
para recuperar el registro deseado de la organización. Si el servicio de organización devuelve una organización, el
objeto de organización devuelto se mapea a un objeto del tipo `OrganizationModelRedis` que se almacena en caché
utilizando el método `cacheOrganizationObject()`.

**Nota**

> Preste mucha atención al manejo de excepciones al interactuar con el caché. Para aumentar la resiliencia, nunca
> permitimos que falle toda la llamada si no podemos comunicarnos con el servidor Redis. En su lugar, registramos la
> excepción y dejamos pasar la llamada al servicio de la organización. En este caso particular, el almacenamiento en
> caché está destinado a ayudar a mejorar el rendimiento y la ausencia del servidor de almacenamiento en caché no
> debería afectar el éxito de la llamada.

Con el código de almacenamiento en caché de Redis implementado, usando curl, podemos seleccionar el servicio de
licencia para ver los mensajes de registro. Si realizamos dos solicitudes GET consecutivas en el punto final del
servicio de licencias, veremos las siguientes dos declaraciones de salida en nuestro registro:

````bash
$ curl -v -H "Authorization: Bearer eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJxTWNNYzlGaFI4bmxPb0xUYkw3R01SVmt1MVJHNmd3b1FhWDJQazBCcFlZIn0..W4ms5xF4tEYYOjKDkYSv0ZsjeWU6e584qfptrM87MYn-dvZxAoF84tbfEu6NY7xQp8Jhk5zZBnrvLXRDyyIIDXbCBScF38f_pNDk9-QVtuWGTMnpEQl0PYyOvmhwZ83rixw2Ack32kFf-vQ3joN_2scWHYdjFJ2cuFi6gyl3JpRGVUlOE7TtpgXAIfppZKL_01X9jNOBFgt2vaohHiCMM-A7tgR_Ky7xo8XMcY4Xxh7B0nXLlX7XZNJpyr8RITYbmyQ81RYx9j4M858hOkRdr2o2nwLxQsW9IqW5HHZfy38zdzlxg9yBrVuSOnI1fNaB0Ci6freA9D006KmbWNvYmw" http://localhost:8072/license/v1/organization/optimaGrowth/license/a9c7901b-6f0c-4ec7-b42c-fcb483062c17/restClient | jq
>
< HTTP/1.1 200 OK
<
{
...
Luego de que se hizo la primera solicitud al endpoint mostrado en el resultado anterior vemos el log
del `licencing-service`:

````bash
[licensing-service] d.m.l.app.service.LicenseService         : Estoy usando restClient como cliente HTTP
[licensing-service] d.m.l.app.client.OrganizationRestClient  : En licensing Service.getOrganization con el id de correlación: 9ade0a3b-686b-422a-9a34-f17ba82df02f
[licensing-service] d.m.l.app.client.OrganizationRestClient  : No se pudo localizar la organización desde la caché de Redis: optimaGrowth
[licensing-service] d.m.l.app.utils.UserContextInterceptor   : [Interceptor] Agregando al header tmx-correlation-id: 9ade0a3b-686b-422a-9a34-f17ba82df02f
[licensing-service] d.m.l.app.utils.UserContextInterceptor   : [Interceptor] Agregando al header tmx-auth-token: null
[licensing-service] d.m.l.app.utils.PropagationInterceptor   : Propagando access_token: eyJhbGciOiJSUzI1NiIsInR5cCIgOiAiSldUIiwia2lkIiA6ICJxTWNNYzlGaFI4bmxPb0xUYkw3R01SVmt1MVJHNmd3b1FhWDJQazBCcFlZIn0..W4ms5xF4tEYYOjKDkYSv0ZsjeWU6e584qfptrM87MYn-dvZxAoF84tbfEu6NY7xQp8Jhk5zZBnrvLXRDyyIIDXbCBScF38f_pNDk9-QVtuWGTMnpEQl0PYyOvmhwZ83rixw2Ack32kFf-vQ3joN_2scWHYdjFJ2cuFi6gyl3JpRGVUlOE7TtpgXAIfppZKL_01X9jNOBFgt2vaohHiCMM-A7tgR_Ky7xo8XMcY4Xxh7B0nXLlX7XZNJpyr8RITYbmyQ81RYx9j4M858hOkRdr2o2nwLxQsW9IqW5HHZfy38zdzlxg9yBrVuSOnI1fNaB0Ci6freA9D006KmbWNvYmw
[licensing-service] d.m.l.app.kafka.LicensingConsumer        : Se recibió el evento GET para el ID de la organización optimaGrowth con id de correlación 9ade0a3b-686b-422a-9a34-f17ba82df02f
````

Después realizamos una segunda petición al mismo endpoint y revisamos el log del `licensing-service`:

````bash
[licensing-service] d.m.l.app.service.LicenseService         : Estoy usando restClient como cliente HTTP
[licensing-service] d.m.l.app.client.OrganizationRestClient  : En licensing Service.getOrganization con el id de correlación: 8d5d22a8-e734-473f-b368-fca18b90b1c2
[licensing-service] d.m.l.app.client.OrganizationRestClient  : Se ha recuperado con éxito una organización optimaGrowth del caché de Redis: OrganizationModelRedis(id=optimaGrowth, name=Casa de la mujer, contactName=Paul Herrera, contactEmail=paul@gmail.com, contactPhone=943857458, port=64222)
````

**Conclusión**

El primer resultado de la consola muestra la primera vez que intentamos acceder al punto final del servicio de licencias
para la organización `optimaGrowth`. El servicio de licencias comprobó la caché de Redis y no pudo encontrar el registro
de la organización que estaba buscando. Luego, el código llamó al servicio de la organización para recuperar los datos.
El siguiente resultado muestra que cuando ingresó al punto final del servicio de licencias por segunda vez, el registro
de la organización ahora está almacenado en caché.
