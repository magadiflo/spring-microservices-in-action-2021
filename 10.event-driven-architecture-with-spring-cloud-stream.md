...
spring:
  kafka:
    producer:
      ...
      properties:
        spring.json.type.mapping: organizationChangeModelToken:dev.magadiflo.organization.app.model.dto.OrganizationChangeModel
````

El objeto que enviaremos al topic de kafka será del tipo `OrganizationChangeModel`, por lo tanto, usamos como
serializador el `JsonSerializer`. Además, estamos mapeando la clase `OrganizationChanelModel` al un token
`organizationChangeModelToken`. De esta manera, cuando implementemos el consumer también usaremos el mismo token y lo
  ...
  kafka:
    consumer:
      ...
      properties:
        ...
        spring.json.type.mapping: organizationChangeModelToken:dev.magadiflo.licensing.app.model.dto.OrganizationChangeModel
````

Notar que, en la configuración `spring.json.type.mapping` estamos usando el mismo token `organizationChangeModelToken`
